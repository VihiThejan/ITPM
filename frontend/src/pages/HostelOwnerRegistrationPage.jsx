...
function HostelOwnerRegistrationPage() {
  const [form, setForm] = useState(defaultForm);
  const [error, setError] = useState("");
  const [message, setMessage] = useState("");
  const [loading, setLoading] = useState(false);
  const { registerHostelOwner } = useAuth();
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await registerHostelOwner(form);
      ...
      <CardContent>
        <Stack spacing={2} component="form" onSubmit={handleSubmit}>
          ...
          <Typography color="text.secondary">
            Register as a hostel owner and verify your account via email OTP.
          </Typography>
...
export default HostelOwnerRegistrationPage;
